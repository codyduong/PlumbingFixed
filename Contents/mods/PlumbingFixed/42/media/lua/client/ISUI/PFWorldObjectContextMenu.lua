...
function ISWorldObjectContextMenu.toggleComboWasherDryer(context, playerObj, object)
  ...
  if ISWorldObjectContextMenu.isSomethingTo(object, playerNum) then
    return
  end
  ...
  if object:isActivated() then
    option = subMenu:addGetUpOption(
      ...
      playerObj,
      ISWorldObjectContextMenu.onToggleComboWasherDryer,
      object
    )
  else
    option = subMenu:addGetUpOption(
  ...
  local label = object:isModeWasher() and getText("ContextMenu_ComboWasherDryer_SetModeDryer")
    or getText("ContextMenu_ComboWasherDryer_SetModeWasher")
  ...
  option = subMenu:addGetUpOption(
    label,
    playerObj,
    ISWorldObjectContextMenu.onSetComboWasherDryerMode,
    object,
    object:isModeWasher() and "dryer" or "washer"
  )
end
...
local function formatWaterAmount(object, setX, amount, max)
  ...
  return string.format(
    "%s: <SETX:%d> %s / %s",
    object:getFluidUiName(),
    setX,
    luautils.round(amount, 2) .. "L",
    luautils.round(max, 2) .. "L"
  )
end
...
ISWorldObjectContextMenu.doDrinkWaterMenu = function(object, player, context)
  ...
  if instanceof(object, "IsoClothingDryer") then
    return
  end
  ...
  local option =
    context:addGetUpOption(getText("ContextMenu_Drink"), worldobjects, ISWorldObjectContextMenu.onDrink, object, player)
...
ISWorldObjectContextMenu.doWashClothingMenu = function(sink, player, context)
  local playerObj = getSpecificPlayer(player)
  if sink:getSquare():getBuilding() ~= playerObj:getBuilding() then
    return
  end
  ...
  if washYourself or washEquipment then
    ...
    if washYourself then
      ...
      local option = mainSubMenu:addGetUpOption(
        getText("ContextMenu_Yourself"),
        playerObj,
        ISWorldObjectContextMenu.onWashYourself,
        sink,
        soapList
      )
      local tooltip = ISWorldObjectContextMenu.addToolTip()
      if soapRemaining < soapRequired then
        ...
        tooltip.description = tooltip.description
          .. getText("IGUI_Washing_Soap")
          .. ": "
          .. round(math.min(soapRemaining, soapRequired), 2)
          .. " / "
          .. tostring(soapRequired)
          .. " <LINE> "
      end
      tooltip.description = tooltip.description
        .. getText("ContextMenu_WaterName")
        .. ": "
        .. round(math.min(waterRemaining, waterRequired), 2)
        .. " / "
        .. tostring(waterRequired)
      ...
      if bodyBlood > 0 then
        tooltip.description = tooltip.description
          ...
          .. ": "
          .. math.ceil(bodyBlood / BloodBodyPartType.MAX:index() * 100)
          .. " / 100"
      end
      if bodyDirt > 0 then
        tooltip.description = tooltip.description
          ...
          .. ": "
          .. math.ceil(bodyDirt / BloodBodyPartType.MAX:index() * 100)
          .. " / 100"
      end
      option.toolTip = tooltip
    ...
    if washEquipment then
      if #washList > 0 then
        ...
        if #washClothing > 0 then
          ...
          option = mainSubMenu:addGetUpOption(
            getText("ContextMenu_WashAllClothing"),
            playerObj,
            ISWorldObjectContextMenu.onWashClothing,
            sink,
            soapList,
            washClothing,
            nil,
        ...
        if #washContainer > 0 then
          ...
          noSoap = soapRequired < soapRemaining
          option = mainSubMenu:addGetUpOption(
            getText("ContextMenu_WashAllContainer"),
            playerObj,
            ISWorldObjectContextMenu.onWashClothing,
            sink,
            soapList,
        ...
        if #washWeapon > 0 then
          ...
          noSoap = soapRequired < soapRemaining
          option = mainSubMenu:addGetUpOption(
            getText("ContextMenu_WashAllWeapon"),
            playerObj,
            ISWorldObjectContextMenu.onWashClothing,
            sink,
            soapList,
        ...
        if #washOther > 0 then
          ...
          noSoap = soapRequired < soapRemaining
          option = mainSubMenu:addGetUpOption(
            getText("ContextMenu_WashAllOther"),
            playerObj,
            ISWorldObjectContextMenu.onWashClothing,
            sink,
            soapList,
      ...
      for i, item in ipairs(washList) do
        ...
        if soapRemaining < soapRequired then
          ...
          tooltip.description = tooltip.description
            .. getText("IGUI_Washing_Soap")
            .. ": "
            .. tostring(math.min(soapRemaining, soapRequired))
            .. " / "
            .. tostring(soapRequired)
            .. " <LINE> "
          noSoap = false
        ...
        tooltip.description = tooltip.description
          .. getText("ContextMenu_WaterName")
          .. ": "
          .. string.format("%.2f", math.min(waterRemaining, waterRequired))
          .. " / "
          .. tostring(waterRequired)
        ...
        if item:IsWeapon() and (item:getBloodLevel() > 0) then
          tooltip.description = tooltip.description
            ...
            .. getText("Tooltip_clothing_bloody")
            .. ": "
            .. math.ceil(item:getBloodLevel() * 100)
            .. " / 100"
        end
        if item:IsClothing() and item:getDirtyness() > 0 then
          tooltip.description = tooltip.description
            ...
            .. getText("Tooltip_clothing_dirty")
            .. ": "
            .. math.ceil(item:getDirtyness())
            .. " / 100"
        end
        local option = mainSubMenu:addGetUpOption(
          getText("ContextMenu_WashClothing", item:getDisplayName()),
          playerObj,
          ISWorldObjectContextMenu.onWashClothing,
          sink,
          soapList,
...
ISWorldObjectContextMenu.onTakeWater = function(worldobjects, waterObject, waterContainerList, waterContainer, player)
  ...
  for i, item in ipairs(waterContainerList) do
    ...
    if item:canStoreWater() and not item:isWaterSource() then
      if
        not didWalk and (not waterObject:getSquare() or not luautils.walkAdj(playerObj, waterObject:getSquare(), true))
      then
        return
      end
      didWalk = true
      ...
      if returnToContainer and (returnToContainer ~= playerInv) then
        ISTimedActionQueue.add(
          ISInventoryTransferUtil.newInventoryTransferAction(playerObj, item, playerInv, returnToContainer)
        )
      end
